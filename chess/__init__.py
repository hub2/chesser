...
def popcount(bb, *, _bin=bin):
    return (POPCOUNT_TABLE16[ bb        & 0xffff] +
            POPCOUNT_TABLE16[(bb >> 16) & 0xffff] +
            POPCOUNT_TABLE16[(bb >> 32) & 0xffff] +
            POPCOUNT_TABLE16[(bb >> 48) & 0xffff])
...
class Board(BaseBoard):
    ...
    def generate_pseudo_legal_moves(self, from_mask=BB_ALL, to_mask=BB_ALL):
        ...
        for from_square in scan_reversed(non_pawns):
            ...
            for to_square in scan_reversed(moves):
                out.append(Move(from_square, to_square))

        # Generate castling moves.
        if from_mask & self.kings:
            out += self.generate_castling_moves(from_mask, to_mask)
        ...
        for from_square in scan_reversed(capturers):
            ...
            for to_square in scan_reversed(targets):
                if square_rank(to_square) in [0, 7]:
                    out.append(Move(from_square, to_square, QUEEN))
                    out.append(Move(from_square, to_square, ROOK))
                    out.append(Move(from_square, to_square, BISHOP))
                    out.append(Move(from_square, to_square, KNIGHT))
                else:
                    out.append(Move(from_square, to_square))
    ...
    def generate_pseudo_legal_ep(self, from_mask=BB_ALL, to_mask=BB_ALL):
        ...
        for capturer in scan_reversed(capturers):
            out.append(Move(capturer, self.ep_square))
        return out
    ...
    def generate_legal_moves(self, from_mask=BB_ALL, to_mask=BB_ALL):
        ...
        if king_mask:
            ...
            out += self.generate_pseudo_legal_moves(from_mask, to_mask)
        return out
    ...
    def generate_castling_moves(self, from_mask=BB_ALL, to_mask=BB_ALL):
        ...
        for candidate in scan_reversed(self.clean_castling_rights() & backrank & to_mask):
            ...
            if not ((self.occupied ^ king ^ rook) & (empty_for_king | empty_for_rook) or
                ...
                out.append(self._from_chess960(self.chess960, msb(king), candidate))
        return out
